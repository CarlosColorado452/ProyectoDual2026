/* ═══════════════════════════════════════════
   ...
   2. Utilidades
   3. LocalStorage (productos, usuarios, sesión)
   4. Inventario (pintar, filtrar, categorías)
   5. Panel lateral
   6. Modal editar
   7. Resumen (menu.html)
   8. Auth (login, registro, permisos)
   9. Eventos
═══════════════════════════════════════════ */
...
/* ════════════════════════════════════════════
   3. LOCALSTORAGE — USUARIOS Y SESIÓN
════════════════════════════════════════════ */
...
function actualizarResumen() {
    ...
    document.getElementById('statCategorias').textContent = new Set(productos.map(p => p.categoria)).size;
    document.getElementById('statStockBajo').textContent = productos.filter(p => p.stock < 5).length;
}
...
if(btnCerrarEvento) btnCerrarEvento.addEventListener('click', cerrarMensaje);

// ── Hamburguesa ──
const hamburguerEvento = document.getElementById('hamburger');
if(hamburguerEvento) hamburguerEvento.addEventListener('click', () => {
    document.getElementById('navMobile').classList.toggle('visible');
});

// ── Filtros ──
const filtrosEvento = document.getElementById('filtrosToggle');
if(filtrosEvento) filtrosEvento.addEventListener('click', () => {
...
if(categoriaToggle) categoriaToggle.addEventListener('click', () => {
    document.getElementById('categoriaContent').classList.toggle('oculto');
    categoriaToggle.classList.toggle('abierto');
});

const categoriaContent = document.getElementById('categoriaContent');
if(categoriaContent) categoriaContent.addEventListener('change', () => {
    pintarInventario(obtenerProductosFiltrados());
});
...
['precioMin', 'precioMax', 'stockMin', 'stockMax'].forEach(id => {
    const el = document.getElementById(id);
    if(el) el.addEventListener('input', () => pintarInventario(obtenerProductosFiltrados()));
});
...
if(btnEditar) btnEditar.addEventListener('click', () => abrirModalEditar(productoEditandoIndex));
...
if(formularioEvento) formularioEvento.addEventListener('submit', (e) => {
    e.preventDefault();
    anadirProducto();
});
